// Client-side MRA fiscalization service for Vite
// Adapted from Next.js server actions

export async function fiscalizeInvoice(invoiceData, isCredit = false) {
  const MRA_API_URL = import.meta.env.VITE_MRA_API_URL || "https://api.mra.mu/fiscalize"
  const MRA_CLIENT_ID = import.meta.env.VITE_MRA_CLIENT_ID
  const MRA_CLIENT_SECRET = import.meta.env.VITE_MRA_CLIENT_SECRET

  if (!MRA_CLIENT_ID || !MRA_CLIENT_SECRET) {
    return {
      success: false,
      error: "MRA credentials not configured. Please set VITE_MRA_CLIENT_ID and VITE_MRA_CLIENT_SECRET in .env file.",
    }
  }
